...
def generate_models(x, y, degs):
    ...
    yVals = np.array(y)
    for degree in degs:
        fits.append(np.polyfit(xVals, yVals, degree))
    return fits
...
def evaluate_models_on_training(x, y, models):
    ...
    """
    for model in models:
        estVals = np.polyval(model, x)
        r2 = r_squared(y, estVals)
        pylab.plot(x, y, 'bo', label = "Measured Values")
        pylab.plot(x, estVals, 'r', \
                   label = "Predictions, R = " + str(round(r2, 5)))
        pylab.xlabel("Year")
        pylab.ylabel("Temperature")
        pylab.title("Linear Regression of " + str(len(model)-1) + " degree(s)\n")
        pylab.legend(loc = 'best')
        pylab.figure()


#test = generate_models([1961, 1962, 1963, 1965],[4.4,5.5,6.6, 10],[1, 2])
#evaluate_models_on_training([1961, 1962, 1963, 1965], [4.4,5.5,6.6, 10], test)

### Begining of program
raw_data = Climate('data.csv')
#print(raw_data.get_yearly_temp('DALLAS', 2011))
## Problem 3
y = []
x = INTERVAL_1
